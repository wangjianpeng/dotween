...
public class BasicsBrain : BrainBase
{
	...
	void Start()
	{
		...
		// Additional tweens //////////////////////////
		// Float
		DOTween.To(()=> floatToTween, x=> floatToTween = x, 100, 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// Int
		DOTween.To(()=> intToTween, x=> intToTween = x, 100, 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// Uint
		DOTween.To(()=> uintToTween, x=> uintToTween = x, 50, 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// Vector2
		DOTween.To(()=> vector2toTween, x=> vector2toTween = x, new Vector2(50,100), 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// Vector4
		DOTween.To(()=> vector4toTween, x=> vector4toTween = x, new Vector4(50,100,150,200), 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// Rect
		DOTween.To(()=> rectToTween, x=> rectToTween = x, new Rect(10, 20, 50, 100), 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// RectOffset
		DOTween.To(()=> rectOffsetToTween, x=> rectOffsetToTween = x, new RectOffset(10, 20, 50, 100), 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// Color
		...
		// String
		DOTween.To(()=> stringToTween1, x=> stringToTween1 = x, "Hello I'm a new string!", 1.5f).SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
		// String (relative)
		DOTween.To(()=> stringToTween2, x=> stringToTween2 = x, "Hello I'm a new string!", 1.5f).SetRelative().SetEase(EaseType.InOutQuad).SetLoops(loops, LoopType.Yoyo).SetAutoKill(false).Pause();
	}
	...
	void OnGUI()
	{
		...
		GUILayout.BeginHorizontal();
		foreach (Tween t in tweens) {
			GUILayout.Label("Position: " + t.Position() + "\nElapsed: " + t.Elapsed() + "\nCompleted Loops: " + t.CompletedLoops());
		}
		GUILayout.EndHorizontal();
